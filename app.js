...
function renderEmojis(emojis) {
  ...
  emojis.forEach((emoji) => {
    const card = document.createElement("div");
    card.className =
      "relative shadow-lg rounded-2xl p-6 flex flex-col items-center justify-center hover:scale-105 transform transition duration-300";


    card.innerHTML = `
      <div class="text-5xl mb-3">${emoji.htmlCode[0]}</div>
      <p class="font-bold text-gray-800 text-center mb-1">${emoji.name}</p>
      <p class="text-sm text-indigo-600 italic mb-2">${emoji.category}</p>
